...
export function realmName(count: number): string {
  if (count < REALM_NAMES.length) return REALM_NAMES[count];
  return `등봉조극 ${count - REALM_NAMES.length + 1}중`;
}

export function rebirthGateMajor(count: number): number {
  if (count === 0) return 7;
  if (count === 1) return 9;
  return 10;
}

export function rebirthBuffPercent(count: number): number {
  return count * 15;
}
