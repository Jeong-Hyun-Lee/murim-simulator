      ...
      <div className="boss-dialog-overlay">
        <div className="boss-dialog-box">
          ...
          <div className="boss-dialog-body">
            ...
            <br />
            +EXP {reward.exp.toLocaleString()} · +전 {reward.gold.toLocaleString()} · +내공 {reward.chi.toLocaleString()}
            {elixirGained > 0 && (
              <>
                <br />+영약 {elixirGained} (대스테이지 최초 클리어)
              </>
            )}
          </div>
          <button className="gong-upgrade-btn" onClick={confirmBossReward}>계속하기</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="boss-dialog-overlay">
        <div className="boss-dialog-box">
          ...
          <button className="gong-upgrade-btn" onClick={confirmBossChallenge}>도전</button>
        </div>
      </div>
    );
  }
