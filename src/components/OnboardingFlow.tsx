...
// wiki/concepts/ux-시나리오-기획서.md 4장(핵심 사용자 플로우) 1~2단계 v1 구현.
    ...
    <div className="onboarding-overlay">
      {step === "intro" ? (
        <div className="onboarding-box">
          <button className="onboarding-skip-btn" onClick={() => setStep("nickname")}>SKIP</button>
          <p className="onboarding-text">{INTRO_CARDS[cardIndex]}</p>
          <button className="gong-upgrade-btn" onClick={nextCard}>다음</button>
        </div>
      ) : (
        <div className="onboarding-box">
          ...
          <div className="equip-detail-actions">
            <button className="gong-upgrade-btn" onClick={() => completeOnboarding(nicknameInput)}>확인</button>
            <button className="panel-close-btn" onClick={() => completeOnboarding("")}>스킵(기본값)</button>
          </div>
        </div>
      )}
    </div>
