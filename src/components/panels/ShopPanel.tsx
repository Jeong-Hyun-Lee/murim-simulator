  ...
  return (
    <div id="general-shop-body">
      <div>
        ...
        <br />
        영약 교환소 — &quot;쇠는 두드릴수록 강해지지만, 영약은 은자로 산다지.&quot; 노(老)씨가
        웃으며 말했다.
      </div>
      <button
        type="button"
...
export const ShopPanel = ({ onClose }: Props) => {
  ...
  return (
    <div id="shop-panel" className="stat-panel">
      ...
      <div id="shop-tabs">
        <button
          type="button"
          className={`shop-tab${tab === 'general' ? ' shop-tab-active' : ''}`}
        ...
        <button
          type="button"
          className={`shop-tab${tab === 'gacha' ? ' shop-tab-active' : ''}`}
