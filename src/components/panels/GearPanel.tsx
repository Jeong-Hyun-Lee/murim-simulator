...
export const GearPanel = ({ onClose, onNavigate }: Props) => {
  ...
  const enhanceDisabled =
    ...
    maxed ||
    gold < cost ||
    enhanceStones < stoneCost ||
    (useProtection && protectionCharms < 1);
  ...
  return (
    <div id="gear-panel" className="stat-panel">
          ...
          return (
            ...
            >
              ...
              >
                ...
                {item && (
                  <span className="gear-doll-slot-summary">
                ...
                <div className={`gear-popover${BOTTOM_ROW.has(slot) ? ' gear-popover-up' : ''}`}>
                  ...
                  {selectedItem ? (
                    <>
                      ...
                      {!maxed && (
                        <div>
                          강화 성공확률 {Math.round(enhanceSuccessChance(targetLevel) * 100)}% ·
                          비용 전 {cost.toLocaleString()}
                          {stoneCost > 0 && ` + 강화석 ${stoneCost}`}
                        </div>
                      )}
                      {protectEligible && (
                        <label
                          htmlFor="gear-protection-checkbox"
                          style={{ display: 'flex', alignItems: 'center', gap: 6 }}
                        >
                          <input
                            id="gear-protection-checkbox"
                            type="checkbox"
                    ...
                    <>
                      ...
                      <div className="gear-popover-candidates">
                        {candidateItems.map((candidate) => (
                          <div key={candidate.id} className="gong-node">
                            <span
                              className="gong-node-name"
                              style={{ color: GRADE_COLOR[candidate.grade] }}
                            >
                              {candidate.grade} +{candidate.enhanceLevel}
                            </span>
                            <button
                              type="button"
                              className="gong-upgrade-btn"
                              onClick={() => equipItem(candidate.id)}
                            >
                              장착
                            </button>
                          </div>
      ...
      <div className="panel-subheader">
        전체 인벤토리 ({inventory.length}) · 강화석 {enhanceStones} · 보호부적 {protectionCharms}
      </div>
