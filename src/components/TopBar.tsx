...
export const TopBar = ({ onTogglePanel }: Props) => {
  ...
  return (
    <div id="top-bar">
      ...
      <button type="button" className="topbar-btn" onClick={() => onTogglePanel('stage')}>
        사냥터
      </button>
      <button type="button" className="topbar-btn" onClick={() => onTogglePanel('stat')}>
        스탯
      </button>
      {farmReturnStage && (
        <button type="button" className="topbar-btn" onClick={stopFarming}>
          자동 등반 복귀 ({farmReturnStage.major}-{farmReturnStage.sub})
        </button>
      )}
      <button type="button" className="topbar-btn" onClick={() => onTogglePanel('gong')}>
        무공
      </button>
      {gearUnlocked ? (
        <button type="button" className="topbar-btn" onClick={() => onTogglePanel('gear')}>
          장비
        </button>
      ) : (
        <button
          ...
          onClick={() => handleLockedClick('장비 아이템을 처음 획득하면 열립니다.')}
        >
          🔒 장비
        </button>
      )}
      {rebirthUnlocked ? (
        <button type="button" className="topbar-btn" onClick={() => onTogglePanel('rebirth')}>
          환골탈태
        </button>
      ) : (
        <button
      ...
      {sectUnlocked ? (
        <button type="button" className="topbar-btn" onClick={() => onTogglePanel('sect')}>
          문파
        </button>
      ) : (
        <button
          type="button"
          className="topbar-btn topbar-btn-locked"
          onClick={() => handleLockedClick('대2 클리어 후 열립니다.')}
        >
          🔒 문파
        </button>
      )}
      {shopUnlocked ? (
        <button type="button" className="topbar-btn" onClick={() => onTogglePanel('shop')}>
          상점
        </button>
      ) : (
        <button
          type="button"
          className="topbar-btn topbar-btn-locked"
          onClick={() => handleLockedClick('대1 보스 클리어 후 열립니다.')}
        >
          🔒 상점
        </button>
      )}
      <button type="button" className="topbar-btn" onClick={bulkUpgradeAllGong}>
        일괄 연마
      </button>
      <button type="button" className="topbar-btn" onClick={togglePause}>
        {paused ? '재개' : '일시정지'}
      </button>
      <button type="button" className="topbar-btn" onClick={toggleSfx}>
        {sfxEnabled ? '효과음 ON' : '효과음 OFF'}
      </button>
    </div>
  );
};
