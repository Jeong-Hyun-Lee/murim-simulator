...
- 주인공과 적은 모두 약 2.5등신의 귀여운 SD 페인터리 그림체로 통일한다. 적을 5등신 이상 성인 비율로 만들지 않는다. 모든 캐릭터 이미지의 배경을 제거하고 실제 투명 PNG인지 확인한다.

- 주인공(목현)만 전투 캔버스 화면 왼쪽에 배치하고 화면 오른쪽을 바라보게 생성한다.
- 적 캐릭터는 모두 화면 오른쪽에 배치하고 화면 왼쪽을 바라보게 생성한다. 적 스프라이트를 좌우 반전해 이 규칙을 우회하지 않는다.
- 프레임 내부 중심축도 주인공은 왼쪽, 적은 오른쪽에 고정한다. 6모션 전체의 최대 공격 범위(무기·궤적 포함)와 모든 자세를 수용하는 공통 프레임 크기를 정하고 모든 모션에 동일하게 사용한다. 모션마다 전체 그림의 중앙으로 재정렬하거나 여백을 잘라내지 않는다.
