...
- `log.md`의 새 항목은 항상 파일 맨 아래에 추가한다. 기존 항목은 고치지 않되, 폐기된 산출물이 주제인 항목은 삭제한다(아래 폐기 규칙).
- **폐기한 것은 기록으로 남기지 않는다.** 문서·에셋·기능을 폐기하면 "(폐기)" 표기나 취소선으로 흔적을 남기지 말고, 그 대상을 가리키는 서술·위키링크·`index.md` 항목·`log.md` 항목을 전부 제거한다. 남는 문장은 폐기 사실을 언급하지 않고 현재 사실만 말하도록 고친다. 폐기 자체를 알리는 새 로그 항목도 쓰지 않는다. 위키가 폐기 이력으로 계속 길어지는 것을 막기 위함이다.
