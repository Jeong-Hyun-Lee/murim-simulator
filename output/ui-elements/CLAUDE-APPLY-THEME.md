...
## 신규 행동 버튼 적용 (2026-09-23)

세 개의 완성형 행동 버튼을 추가했다. 파일은 고정 문구와 아이콘을 포함한 완성 이미지이므로, 해당 버튼 내부의 보이는 `img`·텍스트를 중복해 표시하지 않는다. 다만 클릭 이벤트, 조건 판정, 동적 수치·배지, 키보드 초점과 접근성 이름은 반드시 기존 HTML에 남긴다.

모든 새 UI 에셋은 **상·하·좌·우 네 방향 모두 실제 본체 외곽선까지 캔버스를 자르고, 투명 외곽 여백을 한 픽셀도 남기지 않는다.** 희미한 그림자·리샘플링 잔상(`alpha < 8`)은 본체 경계로 인정하지 않고 제거한 뒤 자른다. 완성본 검수는 네 변 각각에 `alpha >= 8` 픽셀이 존재하는지 확인한다. CSS에서 `padding`이나 `object-fit: contain`으로 다시 빈 공간을 만들지 말고, 매니페스트의 원본 비율로 배치한다.

| 화면·행동              | 런타임 에셋                         | 적용 방법                                                                                                                                        | 유지할 동적 요소                                                                                                             |
| ---------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| 전투 탭 수련 목표      | `/ui/button/btn-goals.webp`         | `.battle-goal-button` 안에 178×37px 이미지로 둔다. 현재 `label-goals` 표시와 `icon-goal`은 이 완성형 이미지와 교체한다.                          | `claimableGoals` 수만 이미지 오른쪽 바깥의 `.goal-count` 배지로 둔다. 버튼의 `onClick`, `aria-label="수련 목표"`는 유지한다. |
| 전투 탭 환골탈태 가능  | `/ui/button/btn-rebirth-ready.webp` | 조건 충족 시 보이는 진입 버튼을 225×43px 이미지로 바꾸고, 기존 `title-rebirth-ready`와 꺾쇠 문자를 보이는 영역에서 제거한다.                     | `canRebirth` 조건과 `setRebirthOpen(true)` 동작은 그대로 둔다. 이미지 버튼에는 `aria-label="환골탈태 가능"`을 남긴다.        |
| 내 정보 전체 화면 뒤로 | `/ui/button/btn-back-my-info.webp`  | `FullView`의 내 정보 화면 뒤로 버튼에 44×43px으로 사용한다. 기존 보이는 `icon-back`과 `뒤로` 글자는 해당 화면에서만 이 완성형 이미지와 교체한다. | 버튼의 `onClick`, 키보드 초점, `aria-label="뒤로"`는 유지한다. 다른 전체 화면은 기존 공용 뒤로 버튼을 계속 쓴다.             |

`btn-goals`와 `btn-rebirth-ready`는 고정 크기 완성형 이미지다. 반응형 폭 늘리기나 `border-image` 3-slice로 쓰지 말고, 작은 화면에서는 주변 여백을 줄여 원본 비율을 보존한다. 이미지 로드 실패 시 기존 CSS 버튼과 화면 밖 접근성 문구가 대체로 남아야 한다.
