...
| 문파·상점·기연 | `sign-sect-qingyun`, `sign-shop-elixir`, `sign-tower`, `icon-gacha`, `icon-action-donate`, `panel-gacha-card-back`, `label-donate-*`, `label-sect-board`, `label-shop-exchange`, `label-gacha-*`, `title-gacha-rates`, `section-gacha-result` | 간판은 해당 카드·행의 고정 머리로만 쓰고, 비용·확률·결과 등급은 텍스트로 남긴다. `icon-gacha`는 대표 48px이며 버튼 안에서는 20px으로 축소할 수 있다. `icon-action-donate`는 전량 기부 버튼의 20px 보조 표식이다. |
