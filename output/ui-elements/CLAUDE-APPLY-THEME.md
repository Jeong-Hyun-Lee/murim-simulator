...
| 장비 | `icon-slot-*` → `.gear-doll-slot`의 슬롯 종류별 보조 표식, `icon-sort`·`icon-filter` → 장비 정렬·필터 제어, `icon-stat-atk`·`icon-stat-def`·`icon-stat-status-resist` → 장비 능력치 줄, `icon-action-equip`·`icon-action-unequip`·`icon-action-disassemble` → 장착·해제·최적 장착·분해 버튼 | 슬롯 이름·등급·강화 수치는 텍스트로 유지한다. 슬롯 아이콘은 32px, 나머지는 20px이다. |
