"""Build Mokhyeon v5 from coherent motion storyboards.

Unlike the v4 builder, this keeps one transform per motion storyboard.  It
never rescales or recenters an individual frame, and it fails instead of
silently clipping pixels outside the 768px runtime cell.
"""
...
SOURCE = ROOT / 'output' / 'sprite-regeneration' / 'mokhyeon-v5'
OUTPUT = SOURCE
...
def render_motion(
    ...
    # visual centres.  Use the approved idle strip's rear-foot axis for every
    # motion instead of letting sword/ribbon pixels move the runtime pivot.
    ...
    for index, frame in enumerate(frames):
        ...
        left = round(ROOT_X + (min_x - frame_root_x) * scale)
        top = round(GROUND_Y + (min_y - frame_ground) * scale)
...
def save_gifs(cells: dict[str, list[Image.Image]]) -> None:
    for motion, frames in cells.items():
        ...
        frames[0].save(
            OUTPUT / f'mokhyeon-v5-{motion}.gif',
            save_all=True,
        ...
        frames[0].save(
            OUTPUT / f'mokhyeon-v5-{motion}-slow.gif',
            save_all=True,
            append_images=frames[1:],
...
def main() -> None:
    ...
    for motion, spec in MOTIONS.items():
        ...
        cells[motion], transforms[motion] = render_motion(extracted)

    atlas, data = pack(cells)
    OUTPUT.mkdir(parents=True, exist_ok=True)
    ASSET.mkdir(parents=True, exist_ok=True)
    build_review(cells).save(OUTPUT / 'mokhyeon-v5-cells.png')
    save_gifs(cells)
    atlas.save(OUTPUT / 'mokhyeon-v5-sheet.png')
    (OUTPUT / 'mokhyeon-v5-sheet.json').write_text(
        json.dumps(data, ensure_ascii=False, indent=2),
        encoding='utf-8',
    )
    atlas.save(ASSET / 'mokhyeon-v5-sheet.png')
    (ASSET / 'mokhyeon-v5-sheet.json').write_text(
        json.dumps(data, ensure_ascii=False, indent=2),
        encoding='utf-8',
    )
