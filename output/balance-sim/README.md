...
bash output/balance-sim/run.sh 챕터4검토 FINAL_MAJOR=40 MON_FROM=30 MON_HP=1.45   # 대31 이후 성장률 가정
```

환경 변수: `SEED`, `FINAL_MAJOR`, `HOUR_CAP`, `WALL_HOURS`, `MON_FROM`(이 대스테이지 이후에만 아래 성장률 적용), `MON_HP`/`MON_ATK`/`MON_DEF`(대스테이지당 성장률, 기본 1.5/1.333/1.25), `MON_BASE`(몬스터 기초치 배수), `EXP_SCALE`(경험치 배수), `SUMMARY`/`JSON`(출력 형식).
